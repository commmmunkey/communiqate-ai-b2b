...
const HomeListView = ({ item, isFree, showSkillCategory = false }: HomeListViewProps) => {
	useEffect(() => {
	...
	const moduleIDs = useMemo(
		() => String(item.moduleID || '').split(',').map((id: string) => id.trim()),
		[item.moduleID]
	);
		...
		try {
			...
			fetch(
				...
				.then((responseJson) => {
					...
					if (status_ !== 'false') {
						const nextPdfUrl = arrData[0]?.questions?.[0]?.queFile ?? null;
	...
	const handleExistingModule = async (moduleID: string) => {
		try {
			...
			if (status === 'true') {
				...
				localStorage.setItem('LUResultquestions', JSON.stringify(filteredQuestions));
				setModuleId(Number(moduleID));
				if (moduleID === moduleIDs[0] && Number(lesson.userlessonReadingcorrectqus) !== 0) {
					localStorage.setItem('LUResultMarks', lesson.userlessonReadingBand);
					localStorage.setItem('LUResultTotalQuestion', lesson.userlessonReadingtotalqus);
					localStorage.setItem('LUResultCorrectQuestion', lesson.userlessonReadingcorrectqus);
					localStorage.setItem('LUResultModuleId', moduleID);
					navigate('/LUResult');
				} else if (moduleID === moduleIDs[2] && Number(lesson.userlessonListeningcorrectqus) !== 0) {
					localStorage.setItem('LUResultMarks', lesson.userlessonListeningBand);
					localStorage.setItem('LUResultTotalQuestion', lesson.userlessonListeningtotalqus);
					localStorage.setItem('LUResultCorrectQuestion', lesson.userlessonListeningcorrectqus);
					localStorage.setItem('LUResultModuleId', moduleID);
					navigate('/LUResult');
				} else {
	...
	const getModuleIcon = (moduleID: string, hasScore: boolean, score: string) => {
		...
		if (isWriting) return <img className="h-12 w-12 object-contain opacity-100 hover:opacity-80 transition-opacity" src={IconWriting} alt="writing" />;
		if (isSpeaking) return <img className="h-12 w-12 object-contain opacity-100 hover:opacity-80 transition-opacity" src={IconSpeaking} alt="speaking" />;
		if (moduleID === moduleIDs[0]) return <img className="h-12 w-12 object-contain opacity-100 hover:opacity-80 transition-opacity" src={IconReading} alt="reading" />;
		if (moduleID === moduleIDs[2]) return <img className="h-12 w-12 object-contain opacity-100 hover:opacity-80 transition-opacity" src={IconListening} alt="listening" />;
		return <img className="h-12 w-12 object-contain opacity-100" src={IconReading} alt="module" />;
	};
		...
		<div className="m-2 rounded-md flex flex-col bg-background max-w-screen-sm mx-auto">
			<div className="p-2 pl-6 pr-4 rounded-md flex items-center justify-between bg-white border-2 border-primary">
				<div className="flex items-center flex-1">
					<img className="h-8 w-8 object-contain mr-2" src={isFree ? IconFree : IconPremium} alt="icon" />
			...
			<div className="p-4 flex flex-row justify-between mb-4">
				<button onClick={onClickLessonVideo} className="hover:scale-105 transition-transform">
					<img className="h-12 w-12 object-contain opacity-100" src={IconVideo} alt="button" />
				</button>
				<button
					onClick={() => {
				...
				>
					{getModuleIcon(
						moduleIDs[0],
						item.userlessonReadingBand !== null && item.userlessonReadingBand !== '0.00',
				...
				>
					{getModuleIcon(
						moduleIDs[1],
						item.userlessonWritingBand !== null && item.userlessonWritingBand !== '0.00',
				...
				>
					{getModuleIcon(
						moduleIDs[3],
						item.userlessonSpeakingBand !== null && item.userlessonSpeakingBand !== '0.00',
				...
				>
					{getModuleIcon(
						moduleIDs[2],
						item.userlessonListeningBand !== null && item.userlessonListeningBand !== '0.00',
			...
			{isLessonVideoWatch && (
				<VideoPopUp howtouse={false} videoLink={item.LessonFileURL} onClose={closePopup} captionTracks={captions} />
			)}
			{isLessonPDFWatch && <PdfPopUp pdfUrl={pdfUrl} onClose={closePdf} onStartAnswering={startAnswering} />}
			<EvaluationLoading />
		</div>
	);
};
	...
	return (
		<div className="relative flex items-center justify-center group">
			<svg className="absolute top-0 left-0" width="48" height="48">
				<circle cx="24" cy="24" r={radius} stroke="currentColor" strokeWidth="4" fill="none" className="text-gray-300" />
				<circle
					cx="24"
					cy="24"
					r={radius}
			...
			<div className={`h-12 w-12 rounded-full bg-white flex items-center justify-center text-xs font-semibold ${getScoreColor()}`}>
