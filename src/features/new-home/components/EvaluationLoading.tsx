...
const EvaluationLoading = () => {
	...
	useEffect(() => {
		if (loadingState.progress < 25) {
			setCurrentStep(0);
		} else if (loadingState.progress < 50) {
			setCurrentStep(1);
		} else if (loadingState.progress < 75) {
			setCurrentStep(2);
		} else {
			setCurrentStep(3);
		}
	...
	const getStepMessages = () => {
		return ['Processing your response', 'Analyzing content and structure', 'Generating detailed feedback', 'Preparing your results'];
	};
		...
		<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
			<div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-xl">
				<div className="text-center">
					...
					<h3 className="text-xl font-semibold text-gray-800 mb-2">AI Evaluation in Progress</h3>
					<p className="text-gray-600 mb-6">{loadingState.message || 'Analyzing your response'}
						{dots}
					</p>
					<div className="mb-6">
					...
					<div className="text-left space-y-3 text-sm">
						{getStepMessages().map((message, index) => (
							<div key={index} className={`flex items-center ${index <= currentStep ? 'text-gray-700' : 'text-gray-400'}`}>
								...
								<span className={index <= currentStep ? 'font-medium' : ''}>{message}</span>
							</div>
						))}
					</div>
